...
# <br><br>
# Also, as random forest classifier randomizes the formation of the trees and takes some subset of features for tree formation (for each of the estimators formed), it is very probable that the Trade kills feature would have come less number of times during the estimator training and thus, is given less feature importance when compared to the rounds played feature. Moreover, as evident from the plot above, we can see that the feature importance of "qtRoundsPlayed" and "qtTradeKills" is not very far off and the scenario specified above is quite probable. Some might argue that the increase in accuracy could be attributed to this third most important feature which is not probably the case. As random forest classifier creates many trees that are used for classification and thus, covers the features in a more comprehensive way thus, giving importance to most of the features in the dataset. Due to this comprehensiveness of the classifier, a lot more data can be fit well and thus, is able to determine the labels more effectively in the test dataset. So, still, I would say that the feature set involving the 3 most important features by the decision tree classifier makes more sense in comparison to the random forest classifier.
...
# 3. qtTrade
# 
# This is the same 3 most important features as that of the Decision Tree Classifier but the third one differs from the Random Forest Classifier (qtRoundsPlayed instead of qtTrade). I would trust the feature set from Decision Tree Classifier and XG Boost as the better one. This is primarly because of the fact that qtTrade is a known strategy for winning the combat games and is found to be quite useful for winning the game if the players are efficient with it. So, this serves as a better correlation feature in comparison to some arbitrary number of rounds played. Some might argue that more the number of rounds played, higher the probability of winning, but that is not always the case and we don't know that for sure. In comparison to that, the number of trade kills ties up with the game data pretty well and is a known technique. 
# <br><br>
# Also, mathematically speaking, for two out of three classifiers, we get the qtTradeKill feature as one of the 3 most important features in the feature set. Moreover, the accuracy score for XGBoost has increased from the random forest model by around 1.5% which tells us that the 3rd most important feature for the XGBoost model would have the greatest contribution for the accuracy score improvement. Thus, it would follow that the qtTrade feature is at least somewhat more important than the qtRoundsPlayed one. Also, as for Gradient Boosting, a subset of the tuples is taken into consideration rather than subset of the features, it would follow that as the 3rd most important feature is coming out to be the qtTrade one, then that one is coming important for different samples of the dataset. This feature importance score given by the XGBoost will be much more comprehensive in comparison to both the Decision Tree Classifier and the Random Forest Classifer. Thus, we can for sure say that the current feature set having the 3 most important features makes the most logical sense. (mathematically as well as according to the business logic)
# <br>
# Thus, I would choose the current feature set as the best one. 
...
xgb_clf_disp = CalibrationDisplay.from_estimator(xgb_clf, X_test_scaled, y_test, n_bins = 10)
plt.show()
...
xgb_clf_platt_disp = CalibrationDisplay.from_estimator(xgb_platt_calib, X_test_scaled, y_test, n_bins = 10)
plt.show()
...
xgb_clf_isotonic_disp = CalibrationDisplay.from_estimator(xgb_isotonic_calib, X_test_scaled, y_test, n_bins = 10)
plt.show()
...
# As the Brier scores increase marginally after applying the calibration methods, we can surely say that the calibration methods don't help in getting better predicted probabilities. This is bolstered by the fact that the original probabilities were already very near to perfect calibration according to the plot in 4.1. Thus, there was not much scope for improvement in terms of getting better prediction probabilities after applying the calibration methods which can also be supported by the plots provided in 4.2. Thus, for our use case here where the data was quite balanced originally, we can say that calibration methods don't help us in getting better prediction probabilities.
